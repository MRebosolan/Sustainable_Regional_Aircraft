...
def tank_sizing(HYDROGENVOLUME,LENGTH,N,TS0=155,TANK_MATERIAL_DENSITY=1780,EMOD=276*10**9,THERMALEXP=-0.64*10**(-6),MAX_ALLOWABLE_STRESS = 5516000000):
    ...
    ###DESIGN CASES
    TANK_THICKNESS_Cc = (PRESSURE_GAS *R+3.8*HYDROGENVOLUME*70*9.81/(2*LENGTH+4*R))/(abs(MAX_ALLOWABLE_STRESS/FOS_TANK-thermal_stress_c)) #PRESSURIZED AND UNDER THERMAL STRESS
    TANK_THICKNESS_Hc = (PRESSURE_GAS *R+3.8*HYDROGENVOLUME*70*9.81/(2*LENGTH+4*R))/(abs(MAX_ALLOWABLE_STRESS/FOS_TANK-thermal_stress_h))
    TANK_THICKNESS_FILLUPc = (PRESSURE_GAS *R+3.8*HYDROGENVOLUME*70*9.81/(2*LENGTH+4*R))/(abs(MAX_ALLOWABLE_STRESS/FOS_TANK-thermal_stress_fillup))   
    
    TANK_THICKNESS_C = PRESSURE_GAS/2 *R/(abs(MAX_ALLOWABLE_STRESS/FOS_TANK-thermal_stress_c)) #PRESSURIZED AND UNDER THERMAL STRESS
    TANK_THICKNESS_H = PRESSURE_GAS/2 *R/(abs(MAX_ALLOWABLE_STRESS/FOS_TANK-thermal_stress_h))
    TANK_THICKNESS_FILLUP = PRESSURE_GAS/2 *R/(abs(MAX_ALLOWABLE_STRESS/FOS_TANK-thermal_stress_fillup))       

    TANK_THICKNESS=max(TANK_THICKNESS_FILLUP,TANK_THICKNESS_H,TANK_THICKNESS_C,TANK_THICKNESS_Cc,TANK_THICKNESS_Hc,TANK_THICKNESS_FILLUPc)

 
    ###COMPOSITES
    if TANK_MATERIAL_DENSITY==1780:
        TANK_THICKNESS_Cc = (PRESSURE_GAS *R+3.8*HYDROGENVOLUME*70*9.81/(2*LENGTH+4*R))/(abs(MAX_ALLOWABLE_STRESS*math.sin(50*math.pi/180)/FOS_TANK-thermal_stress_c)) #PRESSURIZED AND UNDER THERMAL STRESS
        TANK_THICKNESS_Hc = (PRESSURE_GAS *R+3.8*HYDROGENVOLUME*70*9.81/(2*LENGTH+4*R))/(abs(MAX_ALLOWABLE_STRESS*math.sin(50*math.pi/180)/FOS_TANK-thermal_stress_h))
        TANK_THICKNESS_FILLUPc = (PRESSURE_GAS *R+3.8*HYDROGENVOLUME*70*9.81/(2*LENGTH+4*R))/(abs(MAX_ALLOWABLE_STRESS*math.sin(50*math.pi/180)/FOS_TANK-thermal_stress_fillup))   
        
        TANK_THICKNESS_C = PRESSURE_GAS/2 *R/(abs(MAX_ALLOWABLE_STRESS*math.cos(50*math.pi/180)/FOS_TANK-thermal_stress_c)) #PRESSURIZED AND UNDER THERMAL STRESS
        TANK_THICKNESS_H = PRESSURE_GAS/2 *R/(abs(MAX_ALLOWABLE_STRESS*math.cos(50*math.pi/180)/FOS_TANK-thermal_stress_h))
        TANK_THICKNESS_FILLUP = PRESSURE_GAS/2 *R/(abs(MAX_ALLOWABLE_STRESS*math.cos(50*math.pi/180)/FOS_TANK-thermal_stress_fillup))  
        
        TANK_THICKNESScirc=max(TANK_THICKNESS_Cc,TANK_THICKNESS_Hc,TANK_THICKNESS_FILLUPc)
        TANK_THICKNESSlong=max(TANK_THICKNESS_FILLUP,TANK_THICKNESS_H,TANK_THICKNESS_C)
        
        TANK_THICKNESS=TANK_THICKNESScirc+TANK_THICKNESSlong
    ...
    if IWISHTOLOOP:
        ...
        for boiloff in yilist:
            ...
            if boiloff<0.01*HYDROGENVOLUME/1.072*DENSITY_LH:
                ...
                print('insulation thickness pod ',INSULATION_THICKNESS)
...
def tank_sizing_fuselage(HYDROGENVOLUME, R,N,TS0=155,TANK_MATERIAL_DENSITY=1780,EMOD=276*10**9,THERMALEXP=-0.64*10**(-6),MAX_ALLOWABLE_STRESS = 5516000000):
    ...
    if IWISHTOLOOP:
        ...
        for boiloff in yilist:
            ...
            if boiloff<0.01*HYDROGENVOLUME/1.072*DENSITY_LH:
                INSULATION_THICKNESS=xilist[counter]
                print('Ins aft',INSULATION_THICKNESS)
                break
