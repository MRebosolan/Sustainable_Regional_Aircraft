...
w_person = (93.4+78.1)/2
pax_cabin	= 120*w_person
pax_cabin_percent = pax_cabin *100/MTOW

max_PL	= 15127.	
maxpl_fuel = MTOW-OEW-max_PL


fwd_cargo_max = 1699	
aft_cargo_max = 2352	
cargo_max = max_PL-pax_cabin
if cargo_max > fwd_cargo_max+aft_cargo_max:
    cargo = fwd_cargo_max+aft_cargo_max
else:
    cargo = cargo_max
...
M_zfw = OEW + PL
# M_zfw_lbs = M_zfw * 2.2046226218488
...
#determining MAC's
xlemac = 30*10/21 #m, datum is front of nose
MAC = 6.8*10/21 #m, length of MAC
...
#cg OEW
xcgoew = 15.093761811984747 #meter
cgoew_mac = 24.95440889952892 #percent
cgoew_fuselage = xcgoew*100/fuselage_lenght

#cargo
fwdcargo_begin = 2.1*4.9/1.7
fwdcargo_end = fwdcargo_begin+4.9
aftcargo_begin = 6.75*4.9/1.7
aftcargo_end = aftcargo_begin+6.77
...
def loading():
    ...
    cargo1 = plt.plot(100*(np.array([xcgoew,onlyfwdcargo[0],bothcargo[0]])-xlemac)/MAC,[OEW,onlyfwdcargo[1],bothcargo[1]], label = 'Cargo', marker = 'x', color = 'brown')
    ...
    cargo2 = plt.plot(100*(np.array([xcgoew,onlyaftcargo[0],bothcargo2[0]])-xlemac)/MAC,[OEW,onlyaftcargo[1],bothcargo2[1]], marker = 'x', color = 'cyan')
    ...
    window1 = plt.plot(100*(np.array(window[0])-xlemac)/MAC,window[1], label = 'Window seats', marker = '.', color = 'blue')
    ...
    plt.plot(100*(np.array([aisle[0][-1],fully_loaded[0]])-xlemac)/MAC,[M_zfw, fully_loaded[1]], marker = '^', color = 'magenta', label = 'Fuel')
...
clalpha_tail =  2*np.pi*AR_tail/(2+((4+ ((AR_tail*beta/n)**2)*(1+ (np.tan(stabilizer_sweep)**2)/beta**2))**0.5))
clalpha_tail_degrees = np.radians(clalpha_tail)

clalpha_tailless = clalpha*(1+2.15*(diameter/span))*(wing_area-area_fuselage)/wing_area + (np.pi*diameter**2)/(2*wing_area)
