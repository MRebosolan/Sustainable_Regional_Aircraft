...
S =   Cl2.S                # input.S  [m]
b =   Cl2.b                # input.b  [m]
T_OEI = 0.9*0.5*Cl2.Tto        # Thrust at 1 engine inoperative, 
rho_c = input.rho_c
g = input.g
MTOW = g*Cl2.MTOM
...
def rudder_design(y_engine,T_OEI,S,b,vtail_sweep,taper_v,AR_vtail,x_cg,rho_to,Vmin):
    ...
    while running:  
       ...
       # cf_over_c = 0.3              # Rudder chord/vtail chord
        ...
        A_v = AR_vtail
        A_vf_over_A_v =  1.16         # [TODO] constant value Vtail AR in prsence of fuselage to isolated vtail F 10.14 Roskam VI
